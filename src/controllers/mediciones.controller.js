...
export const updateMedicion = async (req,res) => {
    try {
        ...
        const [result] = await pool.query('Update mediciones set fechaRegistro = IFNULL(?,fechaRegistro), sintomas = IFNULL(?, sintomas), userId = IFNULL(?, userId) where id = ?',[fechaRegistro,sintomas,userId,id])
